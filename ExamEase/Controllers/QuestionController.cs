...
namespace ExamEase.Controllers
{
    ...
    public class QuestionController : Controller
    {
            ...
            try
            {
                ...
                var query = from t1 in db.Questions
                            ...
                            select new QuestionViewModel
                            {
                                ...
                                Questionexcellid=t1.Questionexcellid,
                                AnswerSaved = t8,
        ...
        public QuestionViewModel GetViewModel(string Id, string type)
        {
            ...
            model.Questionexcellid = question.Questionexcellid;
            if (type == "View")
            {
            ...
            //{
                ...
                using (var memoryStream = new MemoryStream())
                {
                        ...
                        errors = util.ValidateColumns(columns, new List<string>
                    {
                        ...
                        "Question Type","Status","Questionexcellid","ExcellKurId","ExcellTestId","VideoAyet"
                    });
                        ...
                        if (errors.Count == 0)
                        {
                            for (int i = 0; i < dtRowsCount; i++)
                            {
                                ...
                                //{
                                    ...
                                    double Questionexcellid = dt.Rows[i].Field<double>("Questionexcellid");
                                    double ExcellKurId = dt.Rows[i].Field<double>("ExcellKurId");
                                    double ExcellTestId = dt.Rows[i].Field<double>("ExcellTestId");

                                     string VideoAyet = dt.Rows[i].Field<string>("VideoAyet");


                            //  string subjectId = db.Subjects.Where(a => a.Name == subject).Select(a => a.Id).FirstOrDefault();

                            string subjectId = db.Subjects.Where(a => a.ExcellKurID == ExcellKurId && a.ExcellTestID==ExcellTestId).Select(a => a.Id).FirstOrDefault();

                            string questionTypeId = "8371C70A-6569-4264-9B10-4F0A05B358C4";//db.QuestionTypes.Where(a => a.Name == questionType).Select(a => a.Id).FirstOrDefault();
                                    ...
                                    questionViewModel.Questionexcellid = Convert.ToInt32(Questionexcellid);
                            ...
                            question.ExcellKurId = Convert.ToInt32(ExcellKurId);
                            question.ExcellTestId = Convert.ToInt32(ExcellTestId);

                            question.VideoAyet = VideoAyet;
                            db.Questions.Add(question);
                                    db.SaveChanges();
                                    successCount++;
                                    ModelState.Clear();
        ...
        [HttpPost]
        public IActionResult ImportAnswer(ImportFromExcel model, IFormFile File)
        {
            //try
            //{
            List<string> errors = new List<string>();
            ...
            AnswerViewModelImport answerViewModel = new AnswerViewModelImport();
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                using (var reader = ExcelReaderFactory.CreateReader(memoryStream))
                {
                    ...
                    errors = util.ValidateColumns(columns, new List<string>
                    {
                        "Questionexcellid","AnswerText",
                        "IsCorrect","AnswerOrder"
                    });
                    ...
                    if (errors.Count == 0)
                    {
                        for (int i = 0; i < dtRowsCount; i++)
                        {
                            ...
                            //decimal markValue = 0;
                            double Questionexcellid = dt.Rows[i].Field<double>("Questionexcellid");
                            string AnswerText = dt.Rows[i].Field<string>("AnswerText");
                            string IsCorrect = dt.Rows[i].Field<string>("IsCorrect");
                            double AnswerOrder = dt.Rows[i].Field<double>("AnswerOrder");


                            string QuestionId = db.Questions.Where(a => a.Questionexcellid == Questionexcellid).Select(a => a.Id).FirstOrDefault();
                            ...
                            answerViewModel.AnswerText = AnswerText;
                            answerViewModel.IsCorrect = Convert.ToBoolean(IsCorrect);
                            answerViewModel.AnswerOrder = Convert.ToInt32(AnswerOrder);
                            errors = ValidateImportRowanswer(answerViewModel);
                            ...
                            answer.AnswerOrder = Convert.ToInt32(AnswerOrder);
                            answer.IsCorrect = IsCorrect == "True" ? true : false;
        ...
        public List<string> ValidateImportRowanswer(AnswerViewModelImport model)
        {
            
            List<string> errors = new List<string>();
            if (string.IsNullOrEmpty(model.AnswerText) || string.IsNullOrEmpty(model.QuestionId) )
            {
                errors.Add(Resource.SomeRequiredFieldsAreEmpty);
            }
            else
            {
                ...
                bool existed = db.Questions.Where(a => a.QuestionTitle == model.QuestionId).Any();
                if (existed)
                {
                    errors.Add($"{Resource.QuestionTitle} {Resource.alreadyexists}");
                }
